...
# Combine historic and future tables
    ...
    for historic_file in historic_files:
        # Derive the corresponding future file name
        ...
        if os.path.exists(future_file):
            try:
                ...
                # Combine the tables by summing the values
                combined_df = pd.concat([historic_df, future_df]).groupby(
                    ["Time", "Region simple", "Concrete waste"], as_index=False
                ).sum()
